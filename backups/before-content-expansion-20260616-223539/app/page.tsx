        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {browseGoals.map((goal) => (
            ...
            >
              ...
              <h3 className="mt-4 text-2xl font-bold text-slate-950">{goal}</h3>
              <p className="mt-3 text-slate-600">
                Browse templates and reference-based prompts tuned for {goal.toLowerCase()} workflows.
              </p>
            </Link>
          ))}
        </div>
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {primaryCategories.map((category) => (
              <Link
                ...
                className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"
              >
                <p className="text-sm font-bold uppercase tracking-[0.2em] text-indigo-600">{category}</p>
                <h3 className="mt-4 text-2xl font-bold text-slate-950">{category} Templates</h3>
                <p className="mt-3 text-slate-600">
                  Explore product visuals, short ad scenes, and prompt structures for {category.toLowerCase()} brands.
                </p>
              </Link>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-5xl px-4 pb-20 sm:px-6 lg:px-8">
        <div className="rounded-[2rem] bg-gradient-to-br from-slate-950 to-indigo-950 p-8 text-white shadow-2xl sm:p-12">
          ...
          <form className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            />
            <button type="button" className="rounded-full bg-white px-6 py-3 font-bold text-slate-950">
              Download Free Pack
            </button>
          </form>
        </div>

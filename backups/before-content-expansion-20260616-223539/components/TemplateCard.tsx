...
export function TemplateCard({ template }: TemplateCardProps) {
  ...
  const valueTags = ["Image Prompt", "Runway Prompt", "Storyboard", "Copy-ready"];
    ...
    <article className="group rounded-[1.75rem] border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      <Link href={`/templates/${template.slug}`} className="block">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          <span className="text-xs font-medium text-slate-500">{template.aspectRatio}</span>
        </div>
      ...
      <div className="mt-5 flex items-center justify-between text-sm">
        ...
        <Link href={`/templates/${template.slug}`} className="font-semibold text-slate-950">
          View template
        </Link>
      </div>
    </article>
